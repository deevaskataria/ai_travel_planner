"""
FESTIVAL DATA — full 211-city dataset, v2 (with date ranges)
==============================================================
Each entry: (festival_name, start_month, start_day, end_month, end_day)

- Fixed-date festivals: start == end (e.g. Bastille Day -> (7,14,7,14))
- Movable festivals: a real, reliable RANGE is given instead of a vague
  "varies" — calculated from the actual rule that governs the date
  (lunar calendar window, Easter-linked formula, "Nth weekday of the
  month" rule, etc.). The name still notes *why* it moves.
- A very small number of festivals (Islamic Hijri calendar events,
  Balinese Pawukon calendar) shift by ~11 days a year or don't map to
  a Gregorian window at all — these are flagged explicitly rather than
  given a fake range.

High confidence -> FESTIVAL_DATA
Moderate confidence -> REVIEW_NEEDED (verify before use)
No confident data -> NO_CONFIDENT_DATA (list of city names only)
"""
...
# ============================================================
# REVIEW NEEDED — moderate (not high) confidence.
# Verify before merging into FESTIVAL_DATA.
# ============================================================
REVIEW_NEEDED = {
    ...
    "Buenos Aires": [("Buenos Aires Tango Festival", 8, 1, 8, 20)],
...
if __name__ == "__main__":
    n_cities_main = len(FESTIVAL_DATA)
    n_entries_main = sum(len(v) for v in FESTIVAL_DATA.values())
    n_cities_review = len(REVIEW_NEEDED)
    n_entries_review = sum(len(v) for v in REVIEW_NEEDED.values())
    print(f"FESTIVAL_DATA: {n_cities_main} cities, {n_entries_main} entries")
    print(f"REVIEW_NEEDED: {n_cities_review} cities, {n_entries_review} entries")
