"""
tools.py - CrewAI tool wrappers for the AI Travel Planner backend logic.

These tools expose the existing recommendation engine, budget prediction,
and currency conversion functions to CrewAI agents, allowing them to
reason about travel data and interact with the same logic the app uses.
"""
...
def recommend_destinations_tool(tags: str, budget_per_day: float, travel_style: str) -> str:
    ...
    Returns:
        str: A formatted list of the top 5 recommended destinations with their match score and cost, or an error message.
    """
    try:
        ...
        for i, (_, row) in enumerate(recs.iterrows(), 1):
            ...
            row_tags = row["tags"]
            lines.append(f"{i}. {city}, {country} — Match: {score:.1f}%, Cost: ${cost}/day, Best season: {season}, Tags: {row_tags}")
...
def predict_budget_tool(duration_days: int, num_travelers: int, travel_style: str, currency: str = "USD") -> str:
    ...
    Args:
        ...
        travel_style (str): The style of travel ("budget", "mid", or "luxury").
        currency (str, optional): Target currency for the output (e.g., "USD", "INR", "EUR", "GBP", "JPY"). Defaults to "USD".
        
    Returns:
        str: A formatted string describing the estimated total cost in the requested currency, or an error message.
    """
    try:
        try:
            model = load_model()
        ...
        return f"Estimated total trip cost: {formatted} ({currency}) for {duration_days} days, {num_travelers} travelers, {travel_style} style"
    except Exception as e:
